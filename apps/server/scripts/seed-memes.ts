...
import { db, meme, type MemeData } from "@/db";
...
async function seedMemes() {
  try {
    ...
    const files = readdirSync(memesDir).filter(file => file.endsWith(".json"));
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (existingMeme) {
          console.log(`⚠️  Meme "${memeData.title}" already exists, skipping...`);
          continue;
        }
